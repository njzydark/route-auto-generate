...
function generateRoute(options = {}) {
  ...
  let routes = filesToRoutes(files, routePrefix, pagesPath, framework, coverRoutes, template);
    ...
    watcher
      .on("add", file => {
        if (standardFileFLag.test(file)) {
          files.add(file);
          routes = filesToRoutes(files, routePrefix, pagesPath, framework, coverRoutes, template);
          fs.writeFileSync(routerPath, setRouterTemplate(routes, template));
        }
      })
      .on("unlink", file => {
...
// 根据获取的页面文件地址输出路由代码
function filesToRoutes(files, routePrefix, pagesPath, framework, coverRoutes, template) {
  files = [...files].sort();
  const routes = files.map(file => {
    ...
    return template.routeTpl(path, component);
  });
